...
class MyTransactionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Contract
        fields = ("id", "title", "price", "status", "role", "other_party_name", "main_photo", "created_at")
...
class ContractDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ("id", "access_code", "qr_code_data", "qr_code_image", "status", "created_at", "updated_at")
...
class ContractCreateSerializer(serializers.ModelSerializer):
    ...
    photos = serializers.ListField(
        child=serializers.ImageField(), write_only=True, required=False, allow_empty=True, max_length=10
    )

    class Meta:
        model = Contract
        fields = ("title", "description", "price", "condition", "photos")
    ...
    def validate_photos(self, value):
        ...
        for photo in value:
            if photo.size > max_size:
                raise serializers.ValidationError(f"La foto {photo.name} excede el tamaño máximo de 5MB")
    ...
    def create(self, validated_data):
        ...
        for index, photo in enumerate(photos_data):
            ContractPhoto.objects.create(contract=contract, image=photo, order=index)
